...
export const abs = (rel: string) => (rel ? `${root}/${rel}` : root);
...
/** Start (or restart) on a notes folder: scan, watch, index. */
export async function startLibrary() {
  root = (await getSetting("root")) ?? "";
  if (!root) return;
  await sync();
  await invoke("watch", { root }).catch((e) => toast(`can't watch folder: ${e}`));
}
...
listen("fs-change", () => {
  ...
  syncTimer = window.setTimeout(sync, 600);
}).then((u) => (unlisten = u));
void unlisten;

let syncing: Promise<void> | null = null;
export function sync(): Promise<void> {
  // Serialize: one scan at a time, and one more queued behind it at most.
  syncing = (syncing ?? Promise.resolve()).then(doSync).catch((e) => toast(`scan failed: ${e}`));
  return syncing;
}
...
async function indexAll() {
  ...
  try {
    for (;;) {
      ...
      await indexFile(todo[0]).catch(async (e) => {
...
async function indexFile(f: FileRow) {
  ...
  try {
    ...
    for (let p = 1; p <= doc.numPages; p++) {
      ...
      const its = tc.items.filter((i) => "str" in i) as { str: string; transform: number[] }[];
      items.push(its);
      pageTexts.push(its.map((i) => i.str).join(" "));
    }
    ...
    await importHighlights(f.id, bytes, items);
...
async function renderThumb(doc: pdfjs.PDFDocumentProxy) {
  ...
  await page.render({ canvas, viewport, annotationMode: pdfjs.AnnotationMode.DISABLE }).promise;
  return canvas.toDataURL("image/jpeg", 0.8);
}

/** Pull highlights that exist in the PDF but not in the app (made in Edge/Adobe, or before a reinstall). */
async function importHighlights(fileId: number, bytes: Uint8Array, items: { str: string; transform: number[] }[][]) {
  let found;
  try {
    found = await readHighlights(bytes);
  } catch {
    return; // encrypted or unparseable: nothing to import
  }
  ...
  for (const h of found) {
    const id = h.ours ? h.key.slice(NM_PREFIX.length) : crypto.randomUUID();
    if (h.ours ? ids.has(id) : keys.has(h.key)) continue;
    await run(
      `INSERT OR IGNORE INTO highlights(id, file_id, page, rects, color_id, text, note, created_at, source_key)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [id, fileId, h.page, JSON.stringify(h.rects), nearest(h.hex), textInRects(items[h.page - 1] ?? [], h.rects), h.note, Date.now(), h.ours ? null : h.key],
    );
  }
}
...
export function scheduleSave(fileId: number) {
  clearTimeout(saveTimers.get(fileId));
  saveTimers.set(fileId, window.setTimeout(() => void saveNow(fileId), 1500));
}

export async function flushSaves() {
  const ids = [...saveTimers.keys()];
  ids.forEach((id) => clearTimeout(saveTimers.get(id)));
  saveTimers.clear();
...
async function saveNow(fileId: number) {
  ...
  try {
    ...
    // Our own write shouldn't trigger a re-index.
    await run(`UPDATE files SET mtime=$2, indexed_mtime=$2, size=$3, hash=$4 WHERE id=$1`, [fileId, mtime, out.length, await sha256(out)]);
  } catch (e) {
    if (!warned.has(fileId)) toast(`highlights are safe in the app, but couldn't save them into the PDF: ${e}`);
    warned.add(fileId);
  }
}
...
export async function movePath(fromRel: string, toRel: string, isDir: boolean) {
  ...
  if (isDir && (toRel + "/").startsWith(fromRel + "/")) return toast("can't put a folder inside itself");
  await flushSaves();
  try {
...
export async function makeFolder(parentRel: string, name: string) {
  ...
  await invoke("make_dir", { path: abs(rel) }).catch((e) => toast(String(e)));
  await sync();
}
